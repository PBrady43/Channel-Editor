...
my $version=' Version 2.01(tkh02) issued 8 July 2016';
...
# extra automatic matching put in xmltv import.  ( and ) skipped, 'plus one hour' -> +1, 'plus' ->+,  
...
sub ImportXMLTV{
    ...
    eval{
        ...
        ($reply, $filename)=GetInput("Give file showing callsign to XMLTVID mapping", $filename);
        return unless  $reply eq 'ok';
        ...
        #REMOVED 8Jul 2016
        #open xmltv listing file
        #$filename=ChooseWildcard($filename);
...
sub ChooseWildcard{
    ...
    (my $fn)=@_;
    mylog(0,"filename=$fn");
    my @names; my $choice; my $explain;my $reply;
    while (1){
        track("filename=$fn");
        @names=glob($fn);
        $choice=undef;
        ...
        if ($#names>15){
            ...
            $box -> add('Label', -text=> "Please choose file to map callsign to XMLTVID\n")->pack;
            for(@names){
                $box -> add('Radiobutton', -text => $_,  -variable  =>\$choice, -value=>$_) 
                    -> pack(-anchor => 'w');
            }
        ...
        ($reply, $fn)=GetInput("$explain\nPlease give filename (you can wildcard)\n",'');
...
sub CreateSDConfig{
    ...
    eval{
        ...
        my $configfile=glob('~/.xmltv/tv_grab_sd_json.conf');
        ...
        for my $source (sort keys %sources){
            ...
            for (keys %ChanData){
                if (($ChanData{$_}{SourceId} == $source) and
                    ...
                    ($ChanData{$_}{XMLTVID} !~ /^\s*$/)) {    #not only spaces
